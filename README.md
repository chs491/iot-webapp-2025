...
- 스마트홈(IoT), ERP, 병원예약, 호텡예약, 인터넷뱅킹, 온라인서점 ...
...
- README.md에도 HTML 태그를 그대로 사용(heading은 적용안됨)
...
- a : 링크
- b, i, small, sub, sup, ins, del : 굵은체, 이탤릭체, 작은글씨, 아래첨자, 위첨자...
- ul / ol, li : 동그라미목록/순번목록, 목록 아이템
- table, tr, th, td : 테이블, 테이블로, 테이블헤더, 테이블컬럼
- form, input, button, select, textarea : 입력양식, 텍스트박스, 버튼, 콤보박스, 여러줄텍스트박스
...
### 반응형 웹
- UTF-8이 적용되기 전 : 한국어웹, 영어웹, 일본어웹 등 같은 웹페이지를 언어별로 개발 필요
- 반응형 웹 이전 : 모바일용, PC용, 태블릿용 등 따로 웹사이트 다중 개발 필요
- 문제 : 하나의 웹페이지가 수정되면 다른 웹페이지들도 다 수정 -> 시간, 돈의 Loss
- 하나의 웹페이지로 기기, 언어문제를 전부 해결하고자 나온 기술중 화면쪽 -> 반응형 웹
...
- 트위터 블루프린트로 시작, 트위터 웹사이트를 꾸미기 위해서 개발 
- 현재 5.3 버전
...
- 유료 템플릿
    ...
    - https://themeforest.net/search/sns%20bootstrap : 필요한 소스코드도 구매 가능
    - https://themes.getbootstrap.com/ : 부트스트랩 공식 유로 템플릿
- 무료 템플릿
